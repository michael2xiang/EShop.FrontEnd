...
namespace EShop.FrontEnd.Services.Mapping
{
    ...
    {
        public static GetProductsByCategoryResponse CreaterProductSearchResultFrom(
            this IEnumerable<Product> productsMatchingRefinement, GetProductsByCategoryRequest request)
        {
            ...
            productSearchResultView.TotalNumberOfPages = NoOfResultPageGiven(request.NumberOfResultsPerPage,
                productSearchResultView.NumberOfTitlesFound);
            productSearchResultView.RefinementGroups = GenarateAvailbableProudctRefinementsFrom(productsFound);
        ...
        private static IEnumerable<ProductSummaryView> CropProductListToSatisfyGivenIndex(int pageIndex, int numberOfResultsPerPage, IEnumerable<ProductTitle> productsFound)
        {
            if (pageIndex > 1)
            {
                ...
                return productsFound.Skip(numToSkip).Take(numberOfResultsPerPage).ConverToProductviews();
            }
            else
            {
                return productsFound.Take(numberOfResultsPerPage).ConverToProductviews();
            }
        }

        private static IEnumerable<RefinementGroup> GenarateAvailbableProudctRefinementsFrom(IEnumerable<ProductTitle> productsFound)
        {
        ...
        private static int NoOfResultPageGiven(int numberOfResultsPerPage, int numberOfTitlesFound)
        {
